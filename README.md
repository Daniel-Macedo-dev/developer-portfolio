...
- **Presença em busca (reauditada em 2026-07-09):** consultas `site:` no Bing e DuckDuckGo não retornaram resultados para o domínio de produção — classificação: *não encontrado nos resultados públicos auditados* (indexação ainda pendente; o domínio `.vercel.app` é novo para os crawlers). Fetches com user agents de crawler (Googlebot, Bingbot, LinkedInBot, Twitterbot, facebookexternalhit) recebem 200 com HTML e metadata completos — não há bloqueio técnico.
- **Search Console (propriedade verificada; sitemap diagnosticado em 2026-07-09):** a propriedade URL-prefix foi verificada por meta tag (`GOOGLE_SITE_VERIFICATION` em produção) e `/sitemap.xml` foi submetido manualmente. O relatório passou a exibir "Não foi possível obter o sitemap", e o teste de URL ao vivo falhou com erro genérico da própria ferramenta. Diagnóstico forense completo do lado público: resposta `200` com `Content-Type: application/xml`, corpo byte-idêntico para browser, Googlebot, Googlebot Smartphone e Bingbot, XML estrito válido (24 URLs, 72 alternates, namespaces corretos), sem BOM, sem `X-Robots-Tag`, sem redirect indevido, sem registro AAAA (nenhuma divergência IPv6), robots.txt permitindo tudo e todas as 24 URLs respondendo `200`. Classificação: falha transitória de ingestão do Search Console — nenhum defeito reproduzível no site. Ação: aguardar reprocessamento e reenviar o mesmo `sitemap.xml` uma única vez pela UI. A única imperfeição real encontrada (priority `0.7000000000000001` por aritmética de ponto flutuante) foi corrigida por determinismo, não como causa do erro. O mesmo sitemap serve para o Bing Webmaster (que aceita importar do Search Console agora que a propriedade está verificada).
- **IndexNow (implementado em 2026-07-08):** decisão de V5 revisitada — sem acesso autenticado ao Bing Webmaster, o IndexNow é o único canal sem credencial para iniciar a descoberta no Bing. Implementação mínima: arquivo de chave estático em `public/` (a chave é pública por definição do protocolo, não é segredo) e uma submissão única das URLs do sitemap via `api.indexnow.org`. Sem lógica por deploy — mudanças futuras continuam dependendo do sitemap. Resultado real da submissão (2026-07-08): as 24 URLs do sitemap foram enviadas e a API respondeu `202 Accepted` — recebida, com validação da chave e eventual indexação pendentes. Submissão aceita ≠ indexação.
...
- Sem acesso autenticado ao Search Console/Bing Webmaster no ambiente de desenvolvimento: a propriedade Google está verificada e o sitemap submetido (ações do dono na UI), mas o reenvio do sitemap e a inspeção de URLs continuam dependendo da UI — o estado do painel não é verificável a partir deste ambiente.
- Speed Insights está ativo e coletando (dados reconsultados via `vercel metrics` em 2026-07-09), mas a amostra segue insuficiente para conclusões: 6 datapoints de LCP em 7 dias, todos Firefox desktop nas rotas `/` e `/en`, concentrados nas janelas das execuções de E2E de produção — assinatura de automação, não de tráfego humano; INP e CLS sem dados. Nenhuma métrica de campo é tratada como RUM real até haver volume orgânico.
...
- O fluxo de jogo do GuessMe com respostas da IA não pôde ser capturado (reverificado em 2026-07-09: a credencial Gemini armazenada localmente segue inválida e inalterada desde 2026-06-03 — a API respondeu `API key not valid`); as telas integradas mostram estados reais pré-partida. Nenhuma resposta de IA foi simulada.

## Direções futuras (não implementadas)

- Reprocessamento do sitemap no Search Console e primeira indexação confirmada
- Importação da propriedade verificada no Bing Webmaster Tools
- Captura do fluxo de jogo do GuessMe com respostas reais da IA (exige credencial Gemini válida)
